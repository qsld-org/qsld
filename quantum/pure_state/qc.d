...
import viz.latex.visualization : VL = Visualization;
import viz.typst.visualization : VT = Visualization;
...
struct QuantumCircuit {
    ...
    */
    ...
    private void collapse_many(int[] qubit_idxs, int mask, int result) {
        ...
        for (int i = 0; i < (1 << this.num_qubits); i++) {
            this.state.elems[i] = this.state.elems[i] / state_norm;
        }
    }
    ...
    */
    ...
    // register
    private string measure_many_internal(int[] qubit_idxs, bool do_collapse) {
        ...
        // make mask to identify qubits which need to be the same
        int mask = 0;
        foreach (idx; qubit_idxs) {
            mask |= (1 << idx);
        }
        ...
        for (int b = 0; b < (1 << qubit_idxs.length); b++) {
            ...
            // make a target to identify which qubits
            // need to be the same in b and the state vector
            // index
            foreach (i, idx; qubit_idxs) {
                int bit = (b >> i) & 1;
                if (bit == 1) {
            ...
            // add probabilities of matching qubits to b probability
            // array
            for (int i = 0; i < (1 << this.num_qubits); i++) {
                if ((i & mask) == target) {
                    b_probs[b] += norm(this.state.elems[i]);
                }
            }
        }
    ...
    */
    ...
    /**
    * Puts a barrier at a point in the circuit, this is only supported by
    * the typst visualization for now.
    */
    void barrier() {
        update_visualization_arr("BAR", []);
    }
    ...
    *
    * params:
    * filename = The filename to use for the image generated by the latex
    *
    * remove_pdf_file = Whether or not to remove the pdf file generated by 
    *                   the latex compiler
    */
    void draw_latex(string filename = "circuit.tex", bool remove_pdf_file = true) {
        string compiler = "pdflatex";
        this.vis_latex = VL(this.visualization_arr, this.num_qubits, this.initial_state_idx);
        this.vis_latex.parse_and_write_vis_arr(filename);
        this.vis_latex.compile_tex_and_cleanup(compiler, filename);
        string[] filename_split = filename.split(".");
        string filename_prefix = filename_split[0];
        this.vis_latex.convert_pdf_to_png(filename_prefix, filename_prefix);
    ...
    /**
    * Draws the circuit which the user created with typst
    *
    * params:
    * filename = The filename to use for the image generated by the typst
    *
    * remove_pdf_file = Whether or not to remove the pdf file generated by 
    *                   the typst compiler
    */
    void draw_typst(string filename = "circuit.typ", bool remove_pdf_file = true) {
        this.vis_typst = VT(this.visualization_arr, this.num_qubits, this.initial_state_idx);
        this.vis_typst.parse_and_write_vis_arr(filename);
        this.vis_typst.compile_typst(filename);
        string[] filename_split = filename.split(".");
        string filename_prefix = filename_split[0];
        this.vis_typst.convert_pdf_to_png(filename_prefix, filename_prefix);
